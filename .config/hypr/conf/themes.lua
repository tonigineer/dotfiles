...
local theme = {
    ...
    hypcursor = { package = "bibata-cursor-theme-bin", name = "Bibata-Modern-Ice" },
...
local function write_file(path, content)
    ...
    if not f then
        hl.exec_cmd(string.format('%s "Failed to write %s"', notify.ERROR, path))
        return
    end
    f:write(content .. "\n")
    f:close()
...
local function check_package(label, package)
    local cmd = 'yay -Qa | grep %s || %s "%s: %s not installed"'
    hl.exec_cmd(string.format(cmd, package, notify.ERROR, label, package))
end
...
check_package("HYPR_CURSOR", theme.hypcursor.package)
...
hl.env("HYPRCURSOR_SIZE", theme.cursor.size)
hl.env("HYPRCURSOR_THEME", theme.hypcursor.name)
