...
def bar_from_percent(
    percent: float, width: int = 8, filled_char: str = "⣿", empty_char: str = "⠀"
) -> str:
...
class DesktopInfo(Window):
        ...
        return Box(
            ...
            children=[
                ...
                Box(
                    ...
                    children=[
                        CategoryLabel(
                            "DATE_TIME:", values.get("datetime", "NA"), spacing=8
                        ),
                        CategoryLabel(
                            "UP_TIME:", values.get("uptime", "NA"), spacing=8
                        ),
                    ],
                ),
                Box(
                    ...
                    children=[
                        CategoryLabel(
                            "CPU:",
                            f"{bar_from_percent(values.get("cpu_usage", 100))}",
                            spacing=8,
                        ),
                        CategoryLabel(
                            "MEM:",
                            f"{bar_from_percent(values.get("ram_usage", 100))}",
                            spacing=8,
                        ),
                        CategoryLabel(
                            "GPU:",
                            f"{bar_from_percent(values.get("gpu_usage", 100))}",
                            spacing=8,
                        ),
                    ],
                ),
                Box(
                    ...
                    children=[
                        CategoryLabel(
                            "CPU_TEMP:",
                            f"{values.get("cpu_temp", 100):0.1f} 󰔄",
                            spacing=8,
                        ),
                        CategoryLabel(
                            "GPU_TEMP:",
                            f"{values.get("gpu_temp", 100):0.1f} 󰔄",
                            spacing=8,
                        ),
                    ],
                ),
                Box(
                    ...
                    children=[
                        CategoryLabel(
                            "/",
                            f"{bar_from_percent(values.get("disk", 100))}",
                            spacing=8,
                        ),
                        CategoryLabel(
                            str(Config.Paths.home_dir),
                            f"{bar_from_percent(values.get("disk_home", 100))}",
                            spacing=8,
                        ),
                        CategoryLabel(
                            "SWAP",
                            f"{bar_from_percent(values.get("disk_swap", 100))}",
                            spacing=8,
                        ),
                    ],
                ),
                Box(
                    ...
                    children=[
                        CategoryLabel(
                            "fabricCORE", f"{values.get("app_cpu", 100)} 󰏰", spacing=8
                        ),
                        CategoryLabel(
                            "fabricMEM",
                            f"{values.get("app_memory", 100)} MiB",
                            spacing=8,
                        ),
                    ],
                ),
    ...
    def get_window_count(self, _, event: HyprlandEvent):
        ...
        monitors = json.loads(
            str(self.connection.send_command("j/monitors").reply.decode())
        )
