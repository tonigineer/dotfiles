...
const wiredControl = Widget.CenterBox({
    ...
    endWidget: Widget.ToggleButton({
        label: Network.wired.bind("state").as(v => v === "activated" ? "󰔡 " : "󰨚 "),
        class_name: Network.wired.state === "activated" ? "on" : "off",
        onToggled: ({ active }) => {
            const widget = wiredControl.end_widget;  // TODO works, but self should be given via onToggled, but how???
            widget.label = active ? "󰔡 " : "󰨚 ";
            widget.class_name = active ? "on" : "off";
            wiredInfos.reveal_child = active;
            Utils.execAsync(["bash", "-c", `nmcli device ${active ? "connect" : "disconnect"} $(nmcli device | grep ethernet | cut -d" " -f1)`]);
        },
    }),
})
...
export const NetworkManager = Widget.Window({
